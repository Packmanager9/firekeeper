...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    class Observer {
        constructor(x, y, radius, color, range = 100, rays = 10, angle = (Math.PI * 2)) {
            this.body = new Circle(x, y, radius, color)
            this.color = color
            this.ray = []
            this.rayrange = range
    ...
    function getFireColor() { // random color that will be visible on  black background
        var letters = '0123456789ABCDEF';
        var color = '#';
        for (var i = 0; i < 2; i++) {
            color += letters[(Math.floor(Math.random() * 2) + 14)];
        }
        for (var i = 0; i < 2; i++) {
    ...
    class Player{
        constructor(){
            this.body = new Circle(300,300, 12, "#444444")
            this.wood = 0
        }
        draw(){
            control(this.body, 3)
    ...
    class Fire{
        constructor(){
            ...
            this.heat = 350
            this.body = new Observer(350,350, 0, "transparent", this.heat+(Math.random()*20), Math.PI*2)
            this.body2 = new Observer(350,350, 0, "transparent", (this.heat/2)+(Math.random()*20), Math.PI*2)
            this.body3 = new Observer(350,350, 0, "transparent", (this.heat/10)+(Math.random()*20), Math.PI*2)
        }
        draw(){
            this.body = new Observer(350,350, 0, "transparent", this.heat+(Math.random()*20), Math.PI*2)
            this.body2 = new Observer(350,350, 0, "transparent", (this.heat/2)+(Math.random()*20), Math.PI*2)
            this.body3 = new Observer(350,350, 0, "transparent", (this.heat/10)+(Math.random()*20), Math.PI*2)
            this.body.obstacles = [...treesb]
            this.body.obstacles.push(this.player.body)
            this.body.raymake = 300
            this.body2.obstacles = [...treesb]
            this.body2.obstacles.push(this.player.body)
            this.body2.raymake = 300
            this.body3.obstacles = [...treesb]
            this.body3.obstacles.push(this.player.body)
            this.body3.raymake = 300
            this.player.draw()
            this.body.draw()
            this.body2.draw()
            this.body3.draw()
            ...
            this.heat-=.1
            if(this.body3.body.doesPerimeterTouch(this.player.body)){
                this.heat += 50 * this.player.wood
                this.player.wood = 0
            }
        }
    }
    ...
    class Trees{
        constructor(){
            this.body = new Circle(-0 + (Math.random()*700),-0 + (Math.random()*700),(Math.random()*8)+8, "green")
            this.marked = 0
        }
        draw(){
            if(this.marked == 0){
                this.body.draw()
                if(this.body.doesPerimeterTouch(fire.player.body)){
                    fire.player.wood+=1
                    this.marked = 1
                }
            }
        ...
        clean(){
            if(this.marked == 1){
                treesb.splice(trees.indexOf(this),1)
